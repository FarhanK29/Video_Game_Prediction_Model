...
# Paths
GAMES_LIST_PATH = Path("data/games_list.csv")      
GAMES_DATA_PATH = Path("data/games_list.csv")  
OUTPUT_PATH     = Path("data/games_list.csv")

# Review filters
MIN_PLAYTIME_HOURS = 5          
MIN_WORDS_PER_REVIEW = 5        
TARGET_REVIEWS_PER_GAME = 1000
...
def fetch_filtered_reviews_sentiment(appid, sid, target_n):
    ...
    while len(collected_scores) < target_n:
        ...
        if data.get("success") != 1:
            print(f"[appid {appid}] request unsuccessful")
            break
...
def main():
    ...
    for i, game in enumerate(games, start=1):
        ...
        avg_sent, count = fetch_filtered_reviews_sentiment(appid, sid, TARGET_REVIEWS_PER_GAME)
